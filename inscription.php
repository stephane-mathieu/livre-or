<?php
    ...
    if(!empty(isset($_POST['login'])  && isset($_POST['password']) && isset($_POST['password2']))) {
        $login = $_POST['login'];
        ...
        if (count($result) == 0){
            if ($confirmpassword == $password) {
                $res = mysqli_query($conn, "INSERT INTO utilisateurs(login, password) VALUES ('$login','$password')");
      ...
      <div class=" test container-fluid">
        ...
        <?php }; ?>
          ...
          <form class="d-flex">
            ...
            <button class="btn btn-outline-success text-white " name="submit" type="submit">Search</button>
          </form>
        </div>
      </div>
    </nav>
